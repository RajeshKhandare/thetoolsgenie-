  ...
  return (
    <div className="min-h-screen bg-zinc-50/50 text-zinc-900">
      ...
      <section id="tools" className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        {/* Clickable Tool Cards Grid */}
        <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 pb-20">
          {filteredTools.map((tool: ToolMeta) => (
            <Link
              key={tool.slug}
              href={`/tools/${tool.slug}`}
              className="group flex flex-col justify-between rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-sm transition-all hover:border-violet-300 hover:shadow-md hover:-translate-y-0.5 cursor-pointer block text-left"
            >
              <div>
